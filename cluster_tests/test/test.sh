...
cp $PBS_O_WORKDIR/test.py $SCRATCH/$PROJECT/$PBS_ARRAY_INDEX
cp $PBS_O_WORKDIR/input.txt $SCRATCH/$PROJECT/$PBS_ARRAY_INDEX

# Execute
cd $PBS_ARRAY_INDEX
python3 test.py $PBS_ARRAY_INDEX $PBS_JOBID 1> test.out 2> test.err
cd ..
